...
CONSULTAS = {
    "d1_prestadores_por_departamento": """
        SELECT departamento, COUNT(*) as total_prestadores
    ...
    "d1_empleos_por_departamento": """
        SELECT departamento, SUM(num_emp1) as total_empleos
        FROM registro_nacional_turismo
        WHERE estado_rnt = 'ACTIVO'
        GROUP BY departamento
    ...
    """,
    "d1_estado_rnt": """
        SELECT estado_rnt, COUNT(*) as total
        FROM registro_nacional_turismo
        GROUP BY estado_rnt
        ORDER BY total DESC
    """,
    "d1_prestadores_por_año": """
        SELECT ano, COUNT(*) as total_prestadores
        FROM registro_nacional_turismo
        GROUP BY ano
        ORDER BY ano ASC
    """,
    "d2_sitios_arqueologicos_por_departamento": """
        SELECT departamento, COUNT(*) as total_sitios
        FROM sitios_arqueologicos
        GROUP BY departamento
        ORDER BY total_sitios DESC
    """,
    "d2_sitios_arqueologicos_mapa": """
        SELECT nombre_arqueol_gico, departamento,
               municipio, latitud, longitud
        FROM sitios_arqueologicos
        WHERE latitud IS NOT NULL AND longitud IS NOT NULL
    """,
    "d2_sitios_turisticos_patrimonio": """
        SELECT tipo_de_patrimonio, COUNT(*) as total
    ...
    "d3_espacios_por_tipo": """
        SELECT tipo_lugar, COUNT(*) as total
        FROM espacios_cultura
        GROUP BY tipo_lugar
        ORDER BY total DESC
    """,
    "d3_espacios_por_departamento": """
        SELECT departamento, COUNT(*) as total_espacios
        FROM espacios_cultura
        GROUP BY departamento
        ORDER BY total_espacios DESC
    """,
    "d3_espacios_mapa": """
        SELECT nombre_del_lugar, tipo_lugar,
    ...
    "d4_organizaciones_por_departamento": """
        SELECT departamento, COUNT(*) as total_organizaciones
        FROM organizaciones_cultura
        GROUP BY departamento
        ORDER BY total_organizaciones DESC
    """,
    "d4_naturaleza_juridica": """
        SELECT naturaleza_juridica, COUNT(*) as total
        FROM organizaciones_cultura
